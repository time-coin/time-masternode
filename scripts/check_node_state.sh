...
if [ "$FORK_COUNT" -gt 100 ]; then
    ...
    echo "1. Restart timed service: sudo systemctl restart timed"
    echo "2. Monitor logs: sudo journalctl -u timed -f"
    echo "3. If persistent, try: bash scripts/reindex.sh"
elif [ "$REORG_FAIL" -gt 0 ]; then
    echo "⚠️  STATUS: WARNING - Reorganizations failing"
    echo ""
    echo "Check logs for details and consider restarting the node:"
    echo "1. sudo systemctl restart timed"
    echo "2. journalctl -u timed -f"
