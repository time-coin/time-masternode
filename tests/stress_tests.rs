...
fn test_transaction_throughput_1000_per_second() {
    ...
    assert!(
        throughput >= 900.0,
        "Throughput must be >= 900 TXs/sec, got {}",
        throughput
    );
    assert!(
        finalized >= (total_txs * 95 / 100),
        "95%+ TXs should be finalized"
    );
}
...
fn test_block_production_under_load() {
    ...
    assert!(
        elapsed.as_secs() <= 1,
        "Block production should complete in < 1 second"
    );
}
...
fn test_consensus_latency_under_load() {
    ...
    assert!(
        avg_latency <= 500,
        "Average latency must be <= 500ms, got {}ms",
        avg_latency
    );
    assert!(
        max_latency <= 1000,
        "Max latency must be <= 1000ms, got {}ms",
        max_latency
    );
    assert!(
        min_latency >= 200,
        "Min latency must be >= 200ms, got {}ms",
        min_latency
    );
}
...
fn test_reward_distribution_under_load() {
    ...
    for block_num in 0..BLOCKS_PER_PERIOD {
        ...
        let reward = BASE_REWARD_PER_BLOCK + (TXS_PER_BLOCK / 10); // 10% of TX fees
