...
fn test_unequal_weights_attack() {
    ...
    assert!(
        !avalanche.has_consensus(&attacker_block),
        "Attacker alone cannot finalize"
    );
    ...
    assert!(
        avalanche.has_consensus(&honest_block),
        "Honest 3/4 can finalize"
    );
}
...
fn test_byzantine_validator_cannot_block() {
    ...
    assert!(
        avalanche.has_consensus(&block),
        "Honest 3/4 can finalize despite byzantine"
    );
}
