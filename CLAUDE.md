...
| AV6 | Bitmap position drift | Validator sees wrong paid recipients because node set changed between production and validation | Bitmap positions keyed on volatile IP strings | Open: needs collateral-outpoint-keyed bitmaps |
| AV7 | Reward hijack | Node submits blocks paying non-participants | Misconfigured or malicious producer | Auto-bans: 3 violations → temp ban |
...
| AV13 | TLS SNI flood | Rapid TLS connections (wrong/IP SNI) never blacklisted | `Err` branch of TLS setup never called `record_violation()` | Fixed: violation recorded on TLS failure |
...
| AV25 | Free-tier subnet flooding | 15+ Free-tier masternodes from one /24 fill registry; PHASE3 spawns task per node → OOM | No per-/24 registration or reconnect cap | Fixed: per-/24 cap (max 5) in `register()`; PHASE3 reconnect cap (max 3) |
...
| AV27 | Invalid vote signature spam | Forged Ed25519 vote sigs at ~1-3/sec burn CPU | `verify_vote_signature()` returns `Ok(false)` with no violation | ⚠️ Open: needs 3-strike escalating ban |
| AV28 | Unregistered voter spam | Votes for unregistered IDs at ~15/sec burn registry lookups | Same `Ok(false)` return, no violation | ⚠️ Open: rate-limit per peer (10 rejections/60s → 1 violation) |
...
| AV31 | Fork injection via catch-up sync | Attacker controls majority of a restarting node's connections; forked block arrives first during catch-up → node locked onto minority fork; compounds with AV30 to create indefinite fork trap | No prioritization of whitelisted peers during sync; subnet ban not enforced on `BlockResponse` messages | ⚠️ Partially mitigated: pre-ban attacker subnet before restart; full fix needs whitelisted-peer sync priority |
...
- `SynchronizedCycling` (AV3/AV25): `BanSubnet` mitigation not yet in `MitigationAction` or enforcement loop
- `TlsFlood` (AV13): needs per-IP + per-subnet TLS failure rate tracking
- `InvalidVoteSignatureSpam` (AV27): needs violation recording in `verify_vote_signature()` invalid-sig path
- `UnregisteredVoterSpam` (AV28): needs per-peer sliding-window counter in unregistered-voter path
